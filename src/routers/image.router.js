...
class ImageRouter extends BaseRouter {
    constructor() {
        ...
        this.router().post('/waifu-insult', async (req, res) => {
            ...
            templateRequestCache[requestId] = req.body;
            try {
                let file = await imageController.generateWaifuInsult(req.browser, req.config.host, req.config.port, requestId);
                res.set('Content-Type', 'image/png');
                delete templateRequestCache[requestId];
                return res.status(HTTPCodes.OK).send(file);
            } catch (e) {
        ...
        this.router().get('/waifu-insult-template', async (req, res) => {
            try {
                if (!req.query.requestId) {
                    return res.status(HTTPCodes.BAD_REQUEST).json({
                        status: HTTPCodes.BAD_REQUEST,
